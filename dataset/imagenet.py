...
class CustomDataset(Dataset):
    def __init__(self, feature_dir, label_dir):
        ...
        # self.feature_files = [f"{i}.npy" for i in range(3900)]
        # self.label_files = [f"{i}.npy" for i in range(3900)]
    ...
    def __getitem__(self, idx):
        ...
        if self.flip:
            # aug_idx = torch.randint(low=0, high=2, size=(1,)).item()
            # selected = 4 if aug_idx == 0 else 9
