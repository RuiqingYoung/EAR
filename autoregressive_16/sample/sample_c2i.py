...
from autoregressive_16.models.gpt_opt import GPT_models
from autoregressive_16.models.generate import generate
...
def main(args):
    ...
    #class_labels = [780, 436, 980, 980, 980, 980, 980, 980]
    #class_labels = torch.randperm(1000)[:32].tolist() 971, 980, 977, 908, 812
    # class_labels = [980, 980, 980, 980,
    #                 980, 980, 980, 980, ]
    class_labels = [980, 980, 980, 980, 
                    780, 780, 780, 780, ]
    ...
    # code_b 是 index tensor，比如 shape = [1, 16, 16]
    code_b = index_sample.view(len(class_labels), 16, 16).long()

    # 1. 查 embedding：获取 codebook 向量（float 类型）
    # vq_model.embedding.weight 是 shape [num_tokens, embed_dim]
    z_q = vq_model.quantize.embedding.weight[code_b]  # shape: [1, 16, 16, embed_dim]

    # 2. 转为 [B, C, H, W] 格式
    z_q = z_q.permute(0, 3, 1, 2).contiguous()  # shape: [1, embed_dim, 16, 16]

    # 3. 送入 decode() 解码
    samples = vq_model.decode(z_q)  # 输出应是 [8, 3, 256, 256]

    samples = samples.detach().cpu()  # 取第一个样本，转到 CPU，并且不参与计算图
    #samples = samples.mul(0.5).add(0.5)  # 先乘标准差，再加均值
    # 转成 HWC 格式，方便 matplotlib 显示
