...
def main(args):
    ...
    for x, y in loader:
        x = x.to(device)
        if args.ten_crop:
            x_all = x.flatten(0, 1) #(10B,3,256,256)
            num_aug = 10
        else:
        ...
        with torch.no_grad():
            ...
            rough = F.interpolate(x_all, size=(64,64), mode='area') #(10B,3,64,64)
            _, _, [_, _, rough_indices] = vq_model.encode(rough)

        rough_indices = rough_indices.reshape(x.shape[0], num_aug, -1) #(B,10,4*4)
        codes = indices.reshape(x.shape[0], num_aug, -1) #(B,10,16*16)
        codes = torch.cat([rough_indices,codes], dim=-1)
