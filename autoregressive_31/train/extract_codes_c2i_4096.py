...
def main(args):
    ...
    # 允许的字段
    allowed_keys = {
        "codebook_size", "codebook_embed_dim", "codebook_l2_norm", "codebook_show_usage",
        "commit_loss_beta", "entropy_loss_ratio", "encoder_ch_mult", "decoder_ch_mult",
        "z_channels", "dropout_p", "v_patch_nums", "enc_type", "dec_type", "semantic_guide",
        "detail_guide", "num_latent_tokens", "encoder_model", "decoder_model", "abs_pos_embed",
        "share_quant_resi", "product_quant", "codebook_drop", "half_sem", "start_drop",
        "sem_loss_weight", "detail_loss_weight", "clip_norm", "sem_loss_scale", "detail_loss_scale",
        "guide_type_1", "guide_type_2", "lfq", "scale", "soft_entropy", "dependency_loss_weight",
        "test_model"
    }

    # 过滤配置
    model_cfg = {k: v for k, v in cfg_dict.items() if k in allowed_keys}

    # 构建模型
    vq_model = VQ_models[args.vq_model](**model_cfg)
    ...
    for x, y in loader:
        ...
        y = y.to(device)
        with torch.no_grad():
            indices = vq_model.img_to_idxBl(x_all,[16])
        codes = indices[0].reshape(x.shape[0], num_aug, -1)
